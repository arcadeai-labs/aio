...
#   1. Tear down this worktree's Docker stack, including its volume. A reviewer
#      worktree is recreated fresh every round, and a leftover `aio-<base>_db-data`
#      volume would outlive it and be inherited by the next worktree that happens
#      to claim the same port block — handing a "clean" review environment a
#      previous round's database. That is precisely the class of bug a fresh
#      worktree exists to catch.
...
if [ -f "$WORKTREE/.env.local" ] && command -v docker >/dev/null 2>&1; then
...
for f in "$CLAIMS"/*; do
  ...
  if [ "$(cat "$f")" = "$WORKTREE" ]; then
    rm -f "$f"
    echo "orca-archive: released AIO_PORT_BASE=$(basename "$f")"
  fi
done
