...
only appear when you hold every open issue at once — and on this project the
cross-cutting failures are the expensive ones, because they are silent. A slice
that renames the brand and a slice that touches the judge's matching can each be
correct and together produce a dashboard that confidently reports zero.

**Anything important goes through the human.** See Escalation.

## Environment facts

Read `.orca/local/env.md` for the repo id, the Run id, worker models, and
whether this project runs on a shared GitHub account. That file is gitignored
...
- anything with meaningful impact on the architecture — **this is the broad
  one**;
- anything that changes what a forker sees first: the README quickstart, the
  compose stack, the seed prompts, the shipped config templates. The fork
  experience is the product;
- any change to the access model. It is the one place here where being wrong is
...
- any change to the prompt set or the brand config, which silently rewrites
  week-over-week history rather than adding to it;
...
orca orchestration worker-start --task <task_id> \
  ...
  --name issue-<N>-<slug> --agent <impl-agent> --model <impl-model> --effort <effort> --setup run --json
```
...
value is that it does not trust the implementer's environment. On this project
the canonical bug is *"the numbers look fine because of what a previous run left
behind"* — leftover JSONL in a gitignored `results/`, or a Postgres volume
shared with another worktree. A reviewer that inherited the environment could
not see either.
...
Tell the implementer to stop its dev servers and run
`docker compose --env-file .env.local down -v` before it reports: the reviewer
holds a different port block, so a still-running stack is a live instance of
unreviewed code.
...
port block and a Docker volume. Read the verdict from the PR comment, not the
worker message.
...
nothing a forker sees first.

*Gate before merge* where the slice sets a contract, changes the fork
